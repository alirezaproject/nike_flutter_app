...
class ProductListScreen extends StatefulWidget {
  final int sort;
  final String seachTerm;
  const ProductListScreen({super.key, required this.sort, this.seachTerm = ''});

  const ProductListScreen.search({super.key, required this.seachTerm, this.sort = ProductSort.latest});
...
class _ProductListScreenState extends State<ProductListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.seachTerm.isEmpty ? 'کفش های ورزشی' : 'نتایج جستجو : ${widget.seachTerm}'),
      ),
      body: BlocProvider<ProductListBloc>(
        create: (context) {
          bloc = sl<ProductListBloc>()..add(ProductListStarted(widget.sort, widget.seachTerm));
          return bloc!;
        },
          ...
          builder: (context, state) {
            if (state is ProductListLoaded) {
              ...
              return Column(
                children: [
                  Visibility(
                    visible: widget.seachTerm.isEmpty,
                    child: Container(
                      height: 56,
                                    ...
                                    Expanded(
                                      child: ListView.builder(
                                        ...
                                        itemBuilder: (BuildContext context, int index) {
                                          ...
                                          return InkWell(
                                            onTap: () {
                                              bloc!.add(ProductListStarted(index, widget.seachTerm));
